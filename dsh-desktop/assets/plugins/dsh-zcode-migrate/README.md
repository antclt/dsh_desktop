...
**工作区归组**：dsh 靠工作区注册表去匹配会话头的 `cwd` —— 迁移只写会话日志，不登记工作区的话
迁来的会话会全部掉进「未分组」。所以真迁之后插件会**自动**把涉及的目录补登记成工作区；已迁过
的会话可以用「登记全部工作区」或每个分组头的「登记工作区」补登记。
...
「跳过（目录已不存在）」一栏，并说明这些会话仍会迁移、但会留在「未分组」。也就是说
**不存在 ≠ 失败**：登记结果分「已登记 / 跳过 / 失败」三栏，红字只剩真失败。
