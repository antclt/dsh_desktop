; ==========================================================================
; DSH Desktop 专用 NSIS 安装模板（vendor 自 tauri-bundler）
; --------------------------------------------------------------------------
...
;
; 与上游的全部差异（搜索「DSH」注释可定位）：
;   1. RestorePreviousInstallLocation：Tauri 自身注册表键为空时，回退探测
;      Electron 线（0.3.x/0.4.x）注册表里的旧安装目录，把 $INSTDIR 预填为
;      旧目录——修复 v0.5.0 升级安装「无法识别旧版安装目录」导致的双安装。
;      检测逻辑全部位于 installerHooks.nsh 的
;      DSH_DETECT_LEGACY_INSTALLDIR 宏（只读注册表 + 只读文件探测）。
; ==========================================================================
Unicode true
...
Function RestorePreviousInstallLocation
  ...
  StrCmp $4 "" +2 0
    StrCpy $INSTDIR $4
  ; DSH：Tauri 自身键（上面的 $4）为空时，回退探测 Electron 线旧目录
  ; 并预填 $INSTDIR。宏体在 installerHooks.nsh，只做只读探测，绝不移动/
  ; 删除旧目录内容，也不影响用户在目录页的手动修改（.onInit 先于目录页）。
  !ifmacrodef DSH_DETECT_LEGACY_INSTALLDIR
    !insertmacro DSH_DETECT_LEGACY_INSTALLDIR
  !endif
FunctionEnd
