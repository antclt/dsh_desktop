...
- **内置会话迁移插件 `dsh-zcode-migrate`**：
  - 一键从本地 zcode 导入历史会话，自动建立工作区对应关系并转换原生 session 格式。
  - 修复严格模式工具调用 Schema（移除内部 boolean required，兼容主流大模型接口规范）。
- **知识中心与图谱记忆默认软禁用（按需启用）**：
  - 默认关闭 `dsh-cardian` 与 `graph-memory`，避免未经配置时自动创建卡片库文件与长期对话杂乱注入。
- **内核升级至 0.1.6-alpha.1**：
  - 同步最新内核能力与插件补丁体系。
- **便携版双轨发布**：
  - 全平台自动化流水线同时构建并发布包含自带 Node 运行时的完整版及超轻量 Lite 便携版。
